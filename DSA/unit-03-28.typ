...
- #strong[B+ Tree:]
  ...
  - Leaves are linked together in a linked list for efficient range
    queries.
- #strong[Key Terms:]
  - #strong[Order (m):] Maximum number of children a node can have.
  - #strong[Internal Node:] Node that has children.
  - #strong[Leaf Node:] Contains actual records (in B+ trees).
  - #strong[Keys:] Values stored to guide the search path.
...
- #strong[B Tree:]
  ...
  - Suitable for databases where updates and searching happen at any
    node.
- #strong[B+ Tree:]
  ...
  - Better for systems requiring many range scans (like file systems and
    databases)
- #strong[Why B+ Tree?]
...
<example-insert-keys-10-20-5-6-12-30-7-17-into-an-empty-b-tree-of-order-3.>
- #strong[Step 1:] Insert `10` \
  Tree is empty → Create leaf node with key 10.

- #strong[Step 2:] Insert `20` \
  Leaf node: `[10, 20]`.

- #strong[Step 3:] Insert `5` \
  Leaf node now has `[5, 10, 20]`. Max keys in leaf reached for order 3
  → split needed.

- #strong[Step 4:] Split leaf:

  - Split into two leaves: `[5, 6]` and `[10, 20]`.
  - Push middle key `10` to the parent.

- #strong[Step 5:] Insert further keys (`6, 12, 30, 7, 17`), follow same
  steps:
...
(#emph[Insert diagrams here showing leaf splits and internal node
updates.];)
...
#emph[Note: This is a simplified demo for teaching purposes. A full B+
tree implementation requires careful management of parent nodes and
recursive splitting.]
...
- #strong[Insertion:]
  - Time complexity: O(log n), involves locating the leaf and possibly
    splitting nodes.
- #strong[Space Complexity:]
  - O(n), as all keys are stored in the leaf nodes.
  - Additional pointers are stored in internal nodes; overhead depends
    on order.
...
- #strong[Key-Value Stores:] \
  Optimized lookups and range queries.

- #strong[Storage Systems and Data Warehouses:] \
...
== 8. Key Takeaways
<key-takeaways>
- B+ tree stores all data at the leaves and internal nodes only keep
  keys.
- Leaf nodes are linked to facilitate efficient range queries.
- Insertion may trigger node splits that propagate upward, potentially
  creating a new root.
- B+ trees are preferred over B trees in systems with heavy range query
  requirements.
...
+ Insert the following keys into an empty B+ Tree of order 4:
  `15, 22, 7, 3, 9, 27, 12`.

  - Draw the tree after each insertion.

+ Explain the difference in node structure between B and B+ trees. Why
  does the B+ tree improve range query performance?

+ Write pseudocode to handle deletion of a key in a B+ tree.

+ Implement a search function for the above C code that returns whether
  a key exists.

+ Analyze what happens during splitting of internal nodes with an
  example.

+ Compare the space utilization differences between B+ trees and B
  trees.

#horizontalrule

#emph[Insert diagrams wherever needed for clarity, especially examples
with split processes and node structures.]
